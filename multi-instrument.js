...
let timeoutval = 200;
let threshold_piano = 20;
let threshold_violin = 50;
...
function draw() {
    ...
    if(double_diff && double_diff>threshold_piano){
      //console.log("NOTE!",double_diff);
      let note = getNote();
      piano.triggerAttackRelease(note);
    }

    if(double_diff && double_diff>threshold_violin){
      //console.log("NOTE!",double_diff);
      let note = getNote();
      violin.triggerAttackRelease(note);
    }
...
async function loadData(){
  ...
  const getViolin = samples => Utils.createSampler(samples['vsco2-violin-arcvib']["wav"]);
  piano = await getPiano(samples);
  violin = await getViolin(samples);
